  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8 sm:py-12 space-y-8">
        {/* Header Box */}
        <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-blue-100 ios-gradient-blue text-white shadow-xl relative overflow-hidden">
          <div className="relative z-10 space-y-3">
            <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-xs font-bold bg-white/25 backdrop-blur-md">
              <Sparkles className="w-3.5 h-3.5" />
              {settings?.categoryType || "แบบฟอร์มส่งผลงานนวัตกรรม"} (ปีการศึกษา {settings?.academicYear || "2569"})
            </span>
            <h1 className="text-2xl sm:text-3xl font-extrabold">
              {settings?.trainingName || "แบบฟอร์มส่งผลงานนวัตกรรม"}
            </h1>
            <p className="text-xs sm:text-sm text-blue-50 max-w-2xl font-medium">
              ส่งผลงานได้สูงสุด {maxUpload} ชิ้นงานตามหัวข้อที่แอดมินกำหนด (แจ้งเตือนไปยัง Telegram อัตโนมัติ)
            </p>
          </div>
        </div>
        ...
        {isSuccess ? (
          ...
          <form onSubmit={handleFormSubmitTrigger} className="glass-panel p-6 sm:p-10 rounded-3xl border border-white space-y-8 shadow-sm bg-white">
            {/* Step 1: Select Grade Level & Teacher Name */}
            <div className="space-y-6">
              <div className="flex items-center gap-2 pb-3 border-b border-slate-100">
                <User className="w-5 h-5 text-blue-600" />
                <h2 className="text-lg font-extrabold text-slate-900">
                  ขั้นตอนที่ 1: เลือกสายชั้น และเลือก/เพิ่มชื่อครูผู้ส่งผลงาน
                </h2>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {/* 1. Grade Level Selection First */}
                <div className="space-y-2 sm:col-span-2 p-4 rounded-2xl bg-blue-50/70 border border-blue-100">
                  <label className="block text-sm font-extrabold text-blue-900 flex items-center gap-1.5">
                    <span>1. เลือกสายชั้นที่สอนก่อน (อ.1 - อ.3 หรือ ป.1 - ป.6)</span>
                    <span className="text-red-500">*</span>
                  </label>
                  <select
                    value={gradeLevel}
                    onChange={(e) => handleGradeLevelChange(e.target.value)}
                    className="w-full px-4 py-3.5 rounded-2xl border border-blue-300 bg-white text-slate-900 font-extrabold text-base focus:ring-2 focus:ring-blue-500 focus:outline-none shadow-xs"
                  >
                    {gradeLevels.map((gl) => (
            ...
            {/* Step 2: Work Slots Selector */}
            {fullName.trim() && (
              <div className="space-y-4 pt-4 border-t border-slate-100">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-extrabold text-slate-900 flex items-center gap-2">
                    <FileText className="w-4 h-4 text-blue-600" />
                    <span>เลือกผลงานที่ต้องการส่ง ({maxUpload} ชิ้นงานตามกำหนดโครงการ)</span>
                  </h3>
                  <span className="text-xs font-semibold text-slate-500">
                    ส่งไปแล้ว {userExistingSubmissions.length} / {maxUpload} ชิ้น
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {Array.from({ length: maxUpload }).map((_, idx) => {
                    ...
                    const slotDefinedTitle = getSlotTitle(idx);

                    return (
                      <div
                        key={idx}
                        onClick={() => handleSelectSlot(idx, existingSub)}
                      ...
                      >
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-extrabold text-xs text-blue-600 bg-blue-100 px-2.5 py-0.5 rounded-lg">
                              ชิ้นงานที่ {idx + 1}
                            </span>
                          ...
                          {existingSub && (
                            <p className="text-xs text-slate-600 font-medium pl-1 line-clamp-1">
                              ส่งแล้ว: {existingSub.projectTitle} ({existingSub.uploadDate})
                            </p>
                          )}
                        </div>
                        ...
                        <div>
                          {existingSub ? (
                            <span className="text-xs font-bold px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 flex items-center gap-1 shrink-0">
                              <Check className="w-3.5 h-3.5" />
                              <span>ส่งแล้ว (กดเพื่อเปลี่ยนไฟล์)</span>
                            </span>
                          ) : (
                            <span className="text-xs font-bold px-3 py-1 rounded-full bg-slate-100 text-slate-500 shrink-0">
                              ยังไม่ได้ส่ง (กดส่งชิ้นนี้)
                            </span>
                          )}
                        </div>
                      </div>
            ...
            {/* Step 3: Work Submission Info & File Upload */}
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
                <div className="flex items-center gap-2">
                  <FileText className="w-5 h-5 text-blue-600" />
                  <h2 className="text-lg font-extrabold text-slate-900">
                    รายละเอียดผลงานชิ้นงานที่ {selectedSlotIndex + 1}
                  </h2>
                </div>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all ${
                      submissionMethod === 'drive'
                        ? "ios-gradient-emerald text-white shadow-xs"
                        : "text-slate-600 hover:text-slate-900"
                    }`}
                  >
              ...
              <div className="space-y-6">
                {/* Fixed Admin Project Title Display */}
                <div className="space-y-2 p-4 rounded-2xl bg-slate-50 border border-slate-200">
                  <label className="block text-xs font-bold text-slate-700 flex items-center justify-between">
                    <span className="flex items-center gap-1">
                      <Lock className="w-3.5 h-3.5 text-blue-600" />
                      <span>ชื่อหัวข้อผลงาน (กำหนดตามโครงการโดยแอดมิน)</span>
                    </span>
                    <span className="text-[10px] text-blue-600 font-bold bg-blue-100 px-2 py-0.5 rounded-md">
                      ตามที่แอดมินกำหนด
                    </span>
                  </label>
                  <div className="p-3 rounded-xl bg-white border border-slate-200 font-extrabold text-sm text-slate-900 shadow-2xs">
                    {getSlotTitle(selectedSlotIndex)}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-slate-700 flex items-center justify-between">
                    <span>รายละเอียดผลงานเพิ่มเติม</span>
                    <span className="text-xs text-slate-400 font-medium">(กรอกหรือไม่กรอกก็ได้ - ไม่บังคับ)</span>
                  </label>
                  <textarea
                    rows={3}
                    placeholder="อธิบายรายละเอียดผลงานเพิ่มเติม เช่น วัตถุประสงค์ หรือ สรุปย่อ (ถ้ามี - กรอกหรือไม่กรอกก็ได้)..."
                    value={description}
                ...
                {submissionMethod === 'file' ? (
                  ...
                  <div className="space-y-3 glass-panel p-5 rounded-3xl border border-emerald-200 bg-emerald-50/50">
                    <label className="block text-sm font-bold text-slate-800 flex items-center gap-2">
                      <HardDrive className="w-4 h-4 text-emerald-600" />
                      <span>วางลิงก์ไฟล์ / โฟลเดอร์จาก Google Drive</span>
                      <span className="text-red-500">*</span>
                    </label>
                    ...
                    <p className="text-xs text-slate-500 font-medium">
                      💡 คำแนะนำ: โปรดตั้งค่าแชร์ไฟล์ใน Google Drive เป็น <span className="font-bold text-emerald-700">"ทุกคนที่มีลิงก์ (Anyone with the link)"</span> เพื่อให้คณะกรรมการสามารถเปิดดูได้
                    </p>

                    {driveFileId && (
                      <div className="p-3 rounded-2xl bg-white border border-emerald-200 space-y-2">
                        <div className="flex items-center gap-2 text-xs font-bold text-emerald-700">
                          <CheckCircle2 className="w-4 h-4" />
                          <span>ตรวจพบ Google Drive File ID: {driveFileId}</span>
                        </div>
                        <div className="aspect-16/9 rounded-xl overflow-hidden border border-slate-200 max-h-48 bg-slate-100 flex items-center justify-center">
                          <iframe
                            src={getGoogleDrivePreviewUrl(driveFileId)}
            ...
            {errorMessage && (
              <div className="p-4 rounded-2xl bg-red-50 border border-red-200 text-red-700 text-sm flex items-center gap-3 font-semibold">
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-end">
              <button
                ...
                className="w-full sm:w-auto px-8 py-3.5 rounded-2xl ios-gradient-blue text-white font-extrabold shadow-lg shadow-blue-500/25 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all hover:scale-[1.02]"
              >
                ...
                <span>
                  {isUploading 
                    ? "กำลังส่งข้อมูล..." 
                    : replacingSubmissionId 
                    ? "อัปเดตส่งแทนที่ผลงานชิ้นเดิม" 
                    : "ส่งผลงานชิ้นงานที่ " + (selectedSlotIndex + 1)}
                </span>
              </button>
            </div>
          </form>
