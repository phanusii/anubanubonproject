...
function certificatePreviewUrl(record: CertificateRecord): string {
  if (record.pdfFileId) return `https://drive.google.com/file/d/${encodeURIComponent(record.pdfFileId)}/preview`;
  const fileId = record.pdfUrl?.match(/\/d\/([^/?]+)/)?.[1] || record.pdfUrl?.match(/[?&]id=([^&]+)/)?.[1];
...
export default function CertificatePage() {
    ...
    try {
      ...
      roundResults.filter((item) => item.certificateLoading).forEach((item) => {
        issueCertificate(item.project.id, fullName)
          ...
          .catch((cause) => setResults((current) => current.map((row) => row.project.id === item.project.id ? { ...row, certificateLoading: false, certificateError: friendlyCertificateError(cause instanceof Error ? cause.message : "") } : row)));
      });
    } catch (cause) {
      setSearched(true);
  ...
  return <div className="min-h-screen flex flex-col"><Navbar /><main className="flex-1 max-w-4xl w-full mx-auto px-4 py-6 sm:py-10 space-y-5">
    <section className="rounded-3xl bg-linear-to-br from-amber-400 to-orange-500 px-6 py-6 text-white shadow-lg flex items-center gap-4">
      <div className="w-14 h-14 rounded-2xl bg-white/20 flex items-center justify-center shrink-0"><Award className="w-8 h-8" /></div>
      <div><h1 className="text-2xl sm:text-3xl font-extrabold">ตรวจสอบเกียรติบัตร</h1><p className="text-sm font-semibold text-amber-50 mt-1">เลือกสายชั้นและชื่อ ระบบจะแสดงผลให้ทันที</p></div>
    </section>
    ...
    {searched && !error && <section className="space-y-4"><div className="flex flex-col sm:flex-row sm:items-end justify-between gap-1"><div><p className="text-xs font-extrabold text-amber-600 uppercase tracking-wide">ผลการตรวจสอบ</p><h2 className="text-xl font-extrabold text-slate-900">{name}</h2></div><p className="text-sm font-semibold text-slate-500">ทั้งหมด {results.length} รอบ</p></div>{results.map((result) => <article key={result.project.id} className="overflow-hidden bg-white rounded-3xl border border-slate-200 shadow-sm">
      <div className="p-5 sm:p-6 flex flex-col sm:flex-row sm:items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-extrabold text-blue-600 mb-1">{result.project.kind === "training" ? "การอบรม" : "โครงการ"} · ปีงบประมาณ {result.project.budgetYear || result.project.academicYear || "-"}</p><h3 className="font-extrabold text-slate-900 leading-relaxed">{result.project.name}</h3></div><span className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-extrabold ${result.submitted === result.required && result.required > 0 ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>ส่งแล้ว {result.submitted}/{result.required}</span></div>
      <div className="border-t border-slate-100 p-5 sm:p-6 bg-slate-50/60">{result.record?.status === "issued" ? <div className="space-y-4"><div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div className="flex gap-3"><CheckCircle2 className="w-7 h-7 text-emerald-600 shrink-0" /><div><h4 className="font-extrabold text-emerald-900">ได้รับเกียรติบัตรแล้ว</h4><p className="text-sm text-emerald-700">เลขที่ {result.record.certificateNumber}</p></div></div>{result.record.pdfUrl && <a href={result.record.pdfUrl} target="_blank" rel="noreferrer" className="inline-flex justify-center items-center gap-2 px-5 py-3 rounded-2xl bg-emerald-600 text-white font-extrabold shadow-sm"><Download className="w-4 h-4" />ดาวน์โหลดเกียรติบัตร</a>}</div>{certificatePreviewUrl(result.record) && <div className="rounded-2xl overflow-hidden border border-slate-200 bg-white"><div className="px-4 py-3 border-b border-slate-100"><h4 className="font-extrabold text-slate-800">ตัวอย่างเกียรติบัตร</h4><p className="text-xs text-slate-500">ตรวจสอบรายละเอียดก่อนดาวน์โหลดหรือพิมพ์</p></div><div className="aspect-[1.414/1] bg-slate-100"><iframe title={`ตัวอย่างเกียรติบัตรของ ${result.record.recipientName}`} src={certificatePreviewUrl(result.record)} className="w-full h-full border-0" loading="lazy" allowFullScreen /></div></div>}</div> : result.certificateLoading ? <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-4 flex items-center gap-3 text-sm font-bold text-emerald-700"><LoaderCircle className="w-5 h-5 animate-spin shrink-0" /><div><p className="font-extrabold">ส่งงานครบแล้ว</p><p className="font-medium">กำลังจัดเตรียมเกียรติบัตร กรุณารอสักครู่</p></div></div> : result.certificateError ? <div className="rounded-2xl bg-rose-50 border border-rose-200 p-4 flex gap-3"><FileWarning className="w-6 h-6 text-rose-500 shrink-0" /><div><h4 className="font-extrabold text-rose-800">ส่งงานครบแล้ว</h4><p className="text-sm font-semibold text-rose-700 mt-0.5">{result.certificateError}</p><button type="button" onClick={() => void search(name)} className="mt-3 px-4 py-2 rounded-xl bg-white border border-rose-200 text-sm font-extrabold text-rose-700">ลองใหม่</button></div></div> : result.missing.length ? <div className="space-y-3"><div className="flex gap-3"><FileWarning className="w-6 h-6 text-amber-600 shrink-0" /><div><h4 className="font-extrabold text-amber-900">ยังส่งไม่ครบ {result.missing.length} ชิ้นงาน</h4><p className="text-sm text-slate-500">ส่งรายการด้านล่างให้ครบเพื่อรับเกียรติบัตร</p></div></div>{result.missing.map((item) => <div key={item.index} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-2xl bg-white border border-slate-200 p-4"><div className="min-w-0"><p className="text-xs font-bold text-rose-500">ชิ้นที่ {item.index + 1}</p><p className="font-bold text-slate-900 leading-relaxed">{item.title}</p></div>{result.project.id === activeProjectId ? <a href={submitLink(item)} className="shrink-0 inline-flex justify-center items-center gap-2 px-4 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-extrabold"><Send className="w-4 h-4" />ส่งชิ้นงาน</a> : <span className="shrink-0 text-xs font-bold text-slate-400">ปิดรับส่งแล้ว</span>}</div>)}</div> : <p className="rounded-2xl bg-white border border-slate-200 p-4 text-sm font-bold text-slate-600">ส่งงานครบแล้ว แต่รอบนี้ยังไม่ได้เปิดใช้งานเกียรติบัตร</p>}</div>
    </article>)}</section>}
  </main><Footer /></div>;
}
