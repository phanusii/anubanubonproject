      ...
      {config && <>
        <div className="grid lg:grid-cols-[380px_minmax(0,1fr)] gap-5 items-start">
          <div className="space-y-3">
            <div className="rounded-2xl border border-slate-200 p-4 space-y-3"><StepTitle number={1} title="เลขที่เกียรติบัตร" done={Boolean(config.budgetYear && config.numberStart)} /><div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><label className="text-[11px] font-bold text-slate-600">ปีงบประมาณ<input value={config.budgetYear} onChange={(e) => setConfig({ ...config, budgetYear: e.target.value })} className="mt-1 w-full px-3 py-2.5 rounded-xl border" /></label><label className="text-[11px] font-bold text-slate-600">เลขเริ่มต้น<input type="number" min="1" value={config.numberStart} onChange={(e) => setConfig({ ...config, numberStart: Number(e.target.value) || 1 })} className="mt-1 w-full px-3 py-2.5 rounded-xl border" /></label></div><p className="text-xs font-bold text-amber-700">ตัวอย่าง: {sampleNumber}</p></div>
