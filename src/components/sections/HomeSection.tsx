...
export default function HomeSection({ onOpenPerson }: { onOpenPerson?: (name: string) => void }) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {filteredSubmissions.map((submission) => (
                <MasonryCard
                  ...
                  onClick={() => (onOpenPerson ? onOpenPerson(submission.fullName) : setActiveSubmission(submission))}
                />
              ))}
            </div>
          )}
